...
const createAndSavePDF = async (html) => {
    try {
        const { uri } = await Print.printToFileAsync({ html, width: 400, height: 1000 });
        console.log("URI ", uri)
        if (Platform.OS === "ios") {
...
const EditableVaccine = () => {
    ...
    const [state, setState] = useState({
        age: [
            "Birth", "6 Weeks", "10 Weeks", "14 Weeks", "6 Months", "5 Years"
        ],
        vaccines: [
            "BCG", "HB 1", "OPV 0",
            ...
            "TCV",
            "Influenza", "Year 1 Dose 1", "Year 2 Dose 2", "Year 3", "Year 4", "Year 5"
        ],
        vac_back: [
            ["BCG", "HB 1", "OPV 0"],
            ["DTwP/DTaP 1", "HiB 1", "IPV 1", "HB 2", "PCV 1", "Rota 1"],
            ...
            <View style={styles.container}>
                <Table style={{ flexDirection: 'row' }} borderStyle={{ borderWidth: 1, borderColor: '#C1C0B9' }}>
                    ...
                    <TableWrapper style={{ width: 80 }}>
                        ...
                        <Row style={{ backgroundColor: '#c8e1ff' }} data={["AGE", "VACCINES", "Due Date", "Given on", "Brand Name"]} widthArr={[60, 110, 70, 70, 90]} />
                        <TableWrapper style={{ flexDirection: 'row' }}>
                            <Col data={state.age} style={styles.head} heightArr={state.vac_back.map(v => v.length * height)} width={60} textStyle={styles.text} />
                            <Col data={state.vaccines} style={styles.title} height={height} width={110} textStyle={styles.titleText}></Col>
                            <Col data={dueOn} style={styles.title} height={height} width={70} textStyle={styles.titleText}></Col>
                            <Col data={givenOn} style={styles.title} height={height} width={70} textStyle={styles.titleText}></Col>
                            <Col data={brands} style={styles.title} height={height} width={90} textStyle={styles.titleText}></Col>
                        </TableWrapper>
                    </TableWrapper>
                </Table>
            </View>
            <Button title="DOWNLOAD" onPress={async () => {
                const table = createTable(state.age, state.vac_back, dueOn, givenOn, brands)
                // console.log(table)
